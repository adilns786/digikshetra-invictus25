...
export default function SearchProperties() {
  ...
  // Fetch approved properties from Firestore
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        ...
        const fetchedProperties = querySnapshot.docs
          .map((doc) => {
            ...
            return {
              ...
              price: data.price || 0,
              size: data.area || 0,
              image: data.propertyImages?.arrayValue?.values?.[0]?.stringValue || 
                     "https://source.unsplash.com/400x300/?house", // Show only the first image
              approved: data.Approved || false
            };
          })
          .filter((property) => property.approved); // Only show approved properties
