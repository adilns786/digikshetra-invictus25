...
export default function BuyerDashboard() {
    ...
    const fetchBlockchainData = async () => {
      try {
        ...
        if (data.blockchain) {
          ...
          setBlockchainProperties(approvedProperties);
  ...
  return (
    <div className="space-y-8 p-4 md:p-6">
      ...
      <h3 className="text-xl font-semibold mt-8">Blockchain Properties</h3>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {blockchainProperties.length > 0 ? (
          blockchainProperties.map((property, i) => (
            <Card key={i} className="overflow-hidden shadow-lg rounded-xl">
              <div className="aspect-video w-full bg-muted relative">
              ...
              <CardContent className="p-4 pt-0">
                <div className="flex justify-between">
                  ...
                  <div>
                    <p className="text-sm font-medium">Area</p>
                    <p className="text-xl font-bold">{property.area} sqft</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium">Type</p>
                    <p className="text-xl font-bold">{property.property_type}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium">Price</p>
                ...
                <div className="mt-4 flex justify-end">
                  <Button asChild>
                    <Link to={`/blockchain-details/${property.dlid}`}>View Details</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
          ...
          <p className="text-center text-gray-500">No blockchain properties found.</p>
        )}
      </div>
    </div>
  );
